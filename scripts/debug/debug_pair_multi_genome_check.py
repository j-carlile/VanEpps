...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run pair-focused multi-genome amplicon and conservation debug checks."
    )
    ...
    parser.add_argument(
        "--genomes",
        nargs="+",
        default=["GCA_050293385.1", "GCA_050360595.1", "GCA_910592365.1"],
    )
...
def collect_hits_for_row(
    ...
    return sorted(
        ...
        key=lambda hit: (
            ...
            -int(hit["window_size"]),
            abs(int(hit["subwindow_start"]) - relevant_range(len(searched_sequence), str(row[end_col]).strip())[0]),
...
def pair_hits(
    hits1: List[Dict[str, object]],
    hits2: List[Dict[str, object]],
    expected_length: int,
) -> List[Dict[str, object]]:
    ...
    return sorted(
        ...
        key=lambda pairing: (
            ...
            abs(pairing["raw_gap"] - expected_length),
        ),
    )
...
def main() -> None:
    ...
    print_header("1. Inputs")
    print(f"terminal_windows.tsv: {INPUT_TSV.resolve()}")
    print(f"FASTA directory: {FASTA_DIR.resolve()}")
    print(f"Target pair: {args.gene1}_{args.end1}-{args.gene2}_{args.end2}")
    print(f"Expected amplicon length: {args.expected_length}")
    ...
    if len(selected_rows) != 2:
        raise SystemExit("Expected exactly two target rows in terminal_windows.tsv.")
    ...
    for genome_id in args.genomes:
        ...
        for row_index, row in selected_rows.iterrows():
            ...
            print(f"{key[0]}_{key[1]} hits found: {len(hits)}")
            for hit in hits[:8]:
                print(
        ...
        pairings = pair_hits(
            hits_by_target[target_pairs[0]],
            hits_by_target[target_pairs[1]],
            args.expected_length,
        )

        if not pairings:
            print("No same-contig pairings found from exact 14-20 bp terminal-side hits.")
            continue
        ...
        for pairing in pairings[:10]:
            ...
            print(
                f"  raw_gap={pairing['raw_gap']} | raw_span={pairing['raw_span']} | "
                f"adjusted_gap={pairing['adjusted_gap']} | adjusted_span={pairing['adjusted_span']} | "
                f"distance_from_expected={pairing['distance_from_expected']}"
            )
            print(
                f"  adjustment formula: {pairing['raw_gap']} - {upstream['terminal_offset']} - "
                f"{downstream['terminal_offset']} = {pairing['adjusted_gap']}"
            )
